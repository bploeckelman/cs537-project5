...
void* scannerWorker(void *data) {
    ...
    while (NULL != fgets(line, MAXPATH, info.file_list)) {
        // Cleanup if shutdown is requested
        if(shutdown){
            free(line);
...
char* get_from_buffer() {
    // Get a copy of the filename from the bounded buffer
...
void* indexerWorker(void *data) {
    ...
    // Cleanly exit thread if we are done
    if (shutdown) {
        return NULL;
    }
...
// ----------------------------------------------------------------------------
void startThreadCollector() {
    // Start collector thread that joins indexer threads for a clean exit
...
void cleanup() {
    ...
    // Cleanup other condition variables and mutexes
    if (pthread_mutex_destroy(&mutex_cond.bb_mutex)) {
        fprintf(stderr, "Failed to destroy bounded buffer mutex.\n");
        exit(1);
    }
    if (pthread_cond_destroy(&mutex_cond.empty)) {
        fprintf(stderr, "Failed to destroy empty condition variable.\n");
        exit(1);
    }
    if (pthread_cond_destroy(&mutex_cond.full)) {
        fprintf(stderr, "Failed to destroy full condition variable.\n");
        exit(1);
    }
}
